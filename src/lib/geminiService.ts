...
**重要**：物理试卷里也会有计算题（用公式算速度、密度、力等），但那是物理不是数学！
...
请返回以下 JSON 格式（仅返回JSON，不要其他文字）：
{
  ...
  "knowledgePoint": "这道题考查的具体物理/数学知识点",
  "confidence": 0.95,
  "rawText": "图片中识别到的所有文字"
}

注意：
- 先找试卷标题栏的科目名称，这才是正确答案
- 如果是物理声现象题目，知识点应写"声音的传播速度"、"超声波与次声波"等
- 如果是物理光的折射题目，知识点应写"光的折射"、"折射率"等
- 数学的一元一次方程和物理的公式计算不是一回事，物理有物理的知识点
